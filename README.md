...
stuff for us. Note how the `sigma` coefficient is already in the right
scale (I estimated one sigma for both groups here, but that’s not
necessary). The averages for both groups get returned (`mu[1]` and
`mu[2]`), along with `mu_diff`, so there’s no need to combine slope and
intercept terms. I also (following Kruschke and others) made it so that
